...
do {
    ...
    switch ($choice) {
        '1' {
            $target = Read-Host "Enter target host for Ping Test (default: $($config.Defaults.TargetHost))"
            if (-not $target) { $target = $config.Defaults.TargetHost }
        ...
        '2' {
            $target = Read-Host "Enter target host for Traceroute (default: $($config.Defaults.TargetHost))"
            if (-not $target) { $target = $config.Defaults.TargetHost }
        ...
        '3' {
            ...
            Run-SpeedTest -LogPath $logPath
        }

        '4' {
            $target = Read-Host "Enter target for Bufferbloat Test (default: $($config.Defaults.TargetHost))"
            if (-not $target) { $target = $config.Defaults.TargetHost }
        ...
        '6' {
            $target = $config.Defaults.TargetHost
            ...
            Run-IPGeolocationTest -LogPath $logPath
            Run-PingTest -Target $target -Count $config.Defaults.PingCount -Delay $config.Defaults.PingDelay -LogPath $logPath
            Run-TracerouteTest -Target $target -LogPath $logPath
